...
- Runner adapters: execute fake, shell, Codex, or passwordless SSH runners behind
  one interface.
- Verifier: discover and run commands by exit code; no discovered commands is a
  failed verification.
...
verify outcome hash, evaluates policy and secret scan, then commits the mission
worktree branch and fast-forwards the configured base branch. The stricter verify
evidence hash is retained for audit without being used as the approval gate.
