...
## Corpus Check
- 165 files · ~668,931 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3187 nodes · 10494 edges · 45 communities detected
- Extraction: 80% EXTRACTED · 20% INFERRED · 0% AMBIGUOUS · INFERRED: 2093 edges (avg confidence: 0.8)
...
1. `writeJson()` - 430 edges
2. `nowIso()` - 332 edges
3. `hashEvidence()` - 218 edges
4. `executeCli()` - 150 edges
...
Cohesion: 0.01
Nodes (227): assertRejected(), candidate(), makeResultRoot(), makeTargetCorpusRepo(), policy(), writeCorpusResult(), writeExternalResult(), writeResultFiles() (+219 more)
...
Cohesion: 0.02
Nodes (111): configPath(), ensureGitignore(), initConfig(), readText(), createStore(), discoverVerifyCommands(), exists(), readPackageJson() (+103 more)

### Community 3 - "Community 3"
Cohesion: 0.03
Nodes (114): phase(), averageEvidenceStrength(), buildBenchmarkPlan(), buildCandidateInventions(), buildClaimElementMap(), buildCounterEvidence(), buildExperimentPlan(), buildFactoryScore() (+106 more)
...
Cohesion: 0.04
Nodes (58): auditFromGates(), baselinesForDomain(), candidatesForDomain(), challengeResult(), challengeTask(), clampInt(), exists(), FieldGradeService (+50 more)
...
Cohesion: 0.08
Nodes (30): commandSummary(), containerValidationTest(), energyAuditorPython(), energyAuditorTestsPython(), energyDataset(), EnergyRecordAuditorResearchService, renderClaimFeatureMatrix(), renderCounterEvidence() (+22 more)
...
- **Are the 429 inferred relationships involving `writeJson()` (e.g. with `theoryFixture()` and `writePriorResult()`) actually correct?**
  _`writeJson()` has 429 INFERRED edges - model-reasoned connections that need verification._
- **Are the 331 inferred relationships involving `nowIso()` (e.g. with `.check()` and `.demo()`) actually correct?**
  _`nowIso()` has 331 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 98 inferred relationships involving `executeCli()` (e.g. with `createPublicBetaFixture()` and `createStrictRunWithoutSharedState()`) actually correct?**
  _`executeCli()` has 98 INFERRED edges - model-reasoned connections that need verification._