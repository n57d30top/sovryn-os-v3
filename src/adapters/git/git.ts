...
export class GitAdapter {
  ...
  async isRepo(): Promise<boolean> {
    const result = await runCommand("git rev-parse --is-inside-work-tree", this.root);
    return result.exitCode === 0 && result.stdout.trim() === "true";
  }

  async ensureRepo(): Promise<void> {
    if (!(await this.isRepo())) {
      throw new AppError("GIT_REQUIRED", "Sovryn must run inside a Git work tree.");
    }
  }

  async currentBranch(): Promise<string> {
    const result = await runCommand("git branch --show-current", this.root);
    if (result.exitCode !== 0) throw new AppError("GIT_ERROR", result.stderr || "Unable to read current branch.");
    return result.stdout.trim();
  }

  async hasRef(ref: string): Promise<boolean> {
    const result = await runCommand(`git rev-parse --verify ${shellQuote(ref)}`, this.root);
  ...
  async createWorktree(path: string, branch: string, baseBranch: string): Promise<void> {
    ...
    if (result.exitCode !== 0) throw new AppError("WORKTREE_CREATE_FAILED", result.stderr || result.stdout);
  }

  async removeWorktree(path: string): Promise<void> {
    const result = await runCommand(`git worktree remove --force ${shellQuote(path)}`, this.root);
    if (result.exitCode !== 0) {
      throw new AppError("WORKTREE_REMOVE_FAILED", result.stderr || result.stdout);
    }
  }

  async diffSummary(worktreePath: string, baseBranch: string): Promise<DiffSummary> {
    const status = await runCommand("git status --porcelain=v1", worktreePath);
    if (status.exitCode !== 0) throw new AppError("GIT_STATUS_FAILED", status.stderr || status.stdout);
    ...
    for (const line of status.stdout.split("\n").filter(Boolean)) {
      ...
      if (!byPath.has(path)) {
        const additions = statusCode === "??" ? await countLines(join(worktreePath, path)) : 0;
    ...
    const changedFiles = [...byPath.values()].sort((a, b) => a.path.localeCompare(b.path));
    return {
      changedFiles,
  ...
  async diffPatch(worktreePath: string, baseBranch: string): Promise<string> {
    const result = await runCommand(`git diff --no-ext-diff ${shellQuote(baseBranch)} --`, worktreePath, {
      truncateOutputChars: 50000
    });
    if (result.exitCode !== 0) throw new AppError("GIT_DIFF_FAILED", result.stderr || result.stdout);
  ...
  async commitWorktree(worktreePath: string, message: string): Promise<string | null> {
    ...
    if (add.exitCode !== 0) throw new AppError("GIT_ADD_FAILED", add.stderr || add.stdout);
    const status = await runCommand("git diff --cached --quiet", worktreePath);
    if (status.exitCode === 0) return null;
    const commit = await runCommand(`git commit -m ${shellQuote(message)}`, worktreePath);
    if (commit.exitCode !== 0) throw new AppError("GIT_COMMIT_FAILED", commit.stderr || commit.stdout);
    const rev = await runCommand("git rev-parse HEAD", worktreePath);
    if (rev.exitCode !== 0) throw new AppError("GIT_REV_FAILED", rev.stderr || rev.stdout);
    return rev.stdout.trim();
  }

  async fastForward(baseBranch: string, branch: string): Promise<void> {
    const checkout = await runCommand(`git checkout ${shellQuote(baseBranch)}`, this.root);
    if (checkout.exitCode !== 0) throw new AppError("GIT_CHECKOUT_FAILED", checkout.stderr || checkout.stdout);
    const merge = await runCommand(`git merge --ff-only ${shellQuote(branch)}`, this.root);
    if (merge.exitCode !== 0) throw new AppError("GIT_MERGE_FAILED", merge.stderr || merge.stdout);
  }
}

async function numstat(worktreePath: string, baseBranch: string): Promise<ChangedFile[]> {
  const result = await runCommand(`git diff --numstat ${shellQuote(baseBranch)} --`, worktreePath);
  if (result.exitCode !== 0) throw new AppError("GIT_NUMSTAT_FAILED", result.stderr || result.stdout);
